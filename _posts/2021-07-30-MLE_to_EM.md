...
例如 GMM(2) 可以視為有一個 hidden state $z$ with binary value, 

$$p(x) = p(x\mid z=0) p(z=0) + p(x\mid z=1) p(z=1)$$.  

$p(x\mid z=0)$ 和 $p(x\mid z=1)$ 分別是不同 normal distributions.
...
Lower bound $\eqref{eq1}$ 包含兩個部分： (1) log-likelihood of complete data (both $o$ and $z$) MLE for $\theta$ estimation; (2) $q$ distribution. 
...
例如例四 E-Step 就是計算 $q\left(z_{i}\right)=p\left(z_{i} \mid y_{i}^{\bullet}, \mu_{\mathrm{c}}, \mu_{\mathrm{t}}\right)$ for $i = 1, 2, \cdots, M$
