...
link: 2025-0302-Gaussian_Invarfiant
categories:
  - Science
tags:
  - ML
...
(Input) dataset 一般是 deterministic samples.  
...
上式第一項是 reconstruction loss,  給定 $x$ sample, encoder 產生的 $z$.  encoder 在給定 $z$ 產生 $x$ 的 maximimum likelihood.  如果是 conditional $q(x\vert z)$ 是 Gaussian distribution， maximum likelihood 就是 minimize L2 loss.

第二項則是 KL divergence of encoder 和 Gaussian function 的近似程度。就是 variational loss.
...
這是很有趣的觀點。 $\mathbf{x}$ and $\mathbf{p}$ 都是 deterministic, 甚至 softmax function 都是 deterministic.  但我們賦予最後的 $y$ probabilistic distribution 涵義！基本上 NN 分類網路都是如此 (e.g. VGG, ResNet, MobileNet)。因爲實務可以采樣！
...
(2025/3/2) 最好不要這麽想。而是用 python code 想。基本 softmax 結束之後是一個采樣的過程。如果不是用 greedy algorithm, 或是 temperature = 0,  每次出現的 categorical sample 都不一樣，這次是狗，下次是貓，因此 conditional pdf $p(y\vert x)$  就變成一個 probability distribution!
