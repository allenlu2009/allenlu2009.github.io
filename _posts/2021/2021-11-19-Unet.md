...
而 FCN 提出可以把后面几个全连接 (fully connected layers) 都换成卷积 (convolution), i.e. **fully-connnected 1D 4096/4096/1000 layers 換成 convolutioned 2D 4095/4096/1000 layers.** [longFullyConvolutional2015]  这样就可以获得一张 2D 的 feature map，后接softmax获得每个像素点的分类信息 (2D heat map)，从而解决了分割问题，如下图 (b).
...
使用 1x1 - 3 x 3 - 1 x 1，因此称为瓶颈。 它已在ResNet中使用。

在每次转化前都使用 BN-ReLU，这是激活前ResNet的想法。

（c）基本块

两个 3 x 3卷积，它也用在ResNet中。
...
这里训练集以 30 张[电子显微镜](https://www.zhihu.com/search?q=电子显微镜&search_source=Entity&hybrid_search_source=Entity&hybrid_search_extra={"sourceType"%3A"article"%2C"sourceId"%3A128138930})（EM）图像为数据集，尺寸为 512x512。 25 张图像用于训练，其余5 张图像用于验证。而测试集是另外30 张图像。
