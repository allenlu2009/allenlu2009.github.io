...
Parser 此時除了判斷是否合乎 syntax,  還要決定運算的優先順序。
...
Mistune 的 built-in renderer function 如下。

基本的 markdown 被 cover 在 inline 和 block level.  複雜一點的如 table, footnote, strikethrough 則可用 plugin (also built-in).
...
```markup
block_code(self, code, language=None)
block_quote(self, text)
block_html(self, html)
...
|      | Mistune.Markdown() | Mistune.create_markdown() |
|      |                    |                           |
