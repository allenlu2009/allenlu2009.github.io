...
**Q2: The difference between (statistics) communication estimation theory and learning theory.**
...
Ans:  Excellent YouTube video:  **The Key Equation Behind Probability:**  https://www.youtube.com/watch?v=KHVR587oW8I

![[Pasted image 20240823201916.png]]


- P 是 data 的 (underlying) probability distribution.  實務上當然是用 sample (一般是 0 或 1) 代表。Q 是經過 model 之後的近似 probability distribution。DL(P, Q) 是 H(P, Q) 減掉 H(P) 變成和 data 無關，代表特定 model 所造成的 **average surprise level.**  越小越好。 
- Minimize KL Divergence (= relative entropy) 和 minimize cross-entropy 數學等價，所以我們是用 cross-entropy minimization 得到 model! 

實例1:  fair coin (Q: 50% head, 50% tail), 且擲出的 outcome 是 (P: 51% head, 49% tail)。 KL Loss surprise:  0.51 x log2(0.51/0.5) + 0.49 x log2(0.49/0.5) = 0.0003 bit，**No surprise.**

實例2a:  fair coin (Q: 50% head, 50% tail), 但擲出的 outcome 是 (P: 10% head, 90% tail)。 KL Loss surprise:  0.1 x log2(0.1/0.5) + 0.9 x log2(0.9/0.5) = 0.53 bit，**moderate surprise.**
實例2b:  bias coin (Q: 10% head, 90% tail), 但擲出的 outcome 是 (P: 50% head, 50% tail)。 KL Loss surprise:  0.5 x log2(0.5/0.1) + 0.5 x log2(0.5/0.9) = 0.74 bit，**much surprise!**

實例3a:  fair coin (Q: 50% head, 50% tail), 但擲出的 outcome 是 (P: 1% head, 99% tail)。 KL Loss surprise:  0.01 x log2(0.01/0.5) + 0.99 x log2(0.99/0.5) = 0.92 bit，**very surprise.**
實例3b:  bias coin (Q: 1% head, 99% tail), 但擲出的 outcome 是 (P: 50% head, 50% tail)。 KL Loss surprise:  0.5 x log2(0.5/0.01) + 0.5 x log2(0.5/0.09) = 2.33 bit，**Unbelievable!**

KL Divergence (relative entropy) 就是期待和現實的落差！不過是非對稱的。
沒有期待 bonus (P=fair coin)，實際有 bonus 是 pleasant surprise.
期待大的 bonus (P=bias coin)，實際沒有 bonus 是 unbelievable 且無法接受!!!!

魔術師變魔術也是利用這個手法： 先讓你注意在一個地方 (形成一個 model 或期待)，最後卻是出現在另一個地方。
偵探小說是另一個例子：先讓你以為人是 A 殺的，最後卻是 B 殺的。比起每個人都是嫌疑犯，最後是 B 殺的更刺激！
...
$$
H(X) = \frac{1}{2}\ln (2\pi e \sigma^2) = \frac{1}{2}\ln (2\pi \sigma^2) + \frac{1}{2}
$$
...
以下是对输入（Z）和输出（X）熵以及它们的互信息（MI）在AEP（Asympototic Equipartition Property 渐近均匀性属性）中的角色的进一步说明：

在信息论中，AEP 表示当样本大小足够大时，随机变量的序列将趋向于其熵所定义的典型集 (typical set)。对于输入 Z，其熵 $H_Z$ 可以通过以下关系进行估计：

$$
2^{H_Z} \approx \text{输入典型集}
$$

这意味着，当大量样本被考虑时，Z 的值会集中在一个相对较小的集合中，而这个集合的大小是  $2^{H_Z}$。

同理，对于输出 X，其熵 $H_X$ 也有类似的性质：

$$
2^{H_X} \approx \text{输出典型集}
$$

这里，输出典型集代表了与输入 Z 相关联的一组可能输出。

互信息 (MI) $ I(Z;X) $ 描述了输入 Z 和输出 X 之间共享的信息量，其定义为：

$$
I(Z;X) = H_Z - H_{Z|X}
$$

在上述关系中，$H_{Z|X}$ 是给定 X 的条件熵。互信息越大，表示 Z 和 X 之间的依赖程度越强。

当我们将这些概念结合起来看时，可以得出结论：在极限情况下，**当数据量无限增大时，输入和输出之间的关系会趋向于一种稳定模式**，**此时 AEP 可以帮助我们理解如何从输入的统计特性推导出对应输出的统计特性。通过分析互信息，我们能够深入理解信道传输过程中的信息保留和损失，从而优化通信系统。


另一類問題是從個別 X estimate 個別 Z.  這是 individual sample estimation problem.

* Maximum Likelihood Estimation (MLE), why not MAP?  MAP is biased (by the prior distribution)!

**Is Maximum Log-Likelihood Estimation (individual sample) this same as Maximum Mutual Information (global channel capacity)?** : NO
