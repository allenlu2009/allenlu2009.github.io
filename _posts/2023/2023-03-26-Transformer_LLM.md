...
link: "[[2023-04-09-XYZ_All_U_Need]]"
categories:
  - LLM
tags:
  - AI
...
## B: Matrix Multiplication Rationale
...
* GPT2 : 以下是 Jay Alammar 對 GPT2 的參數估算，似乎一樣??? (2304 = 768x3 KQV,  3072=768x4 MLP)
  ...
  * 爲什麼 token embedding 如此大, 38.6M?  不是因爲 token length 太長！而是支持 byte-level version of  Byte Pair Encoding (BPE) for unicode characters and a **vocabulary size of 50257**.

<img src="/media/image-20230408113653936.png" alt="image-20230408113653936" style="zoom:60%;" />
...
* **Token length 的影響**
  ...
  * **Token length 到底和什麼有関?  transformer 內部計算的中間 result 會有 512 (or 1024)x768 matrix, 所以直接和計算量 (TOPS) 以及內存大小有關。但是和參數無關。**、
