---
title: XYZ Is All You Need Rationale
...
## Attention is all you need rationale

NLP 的問題是 input text string 一般是變動的，例如：“Hello, world",  或是 "This is a test of natural language processing!"

假設 context length or block_size = 512.

Input 是 text string, 切成 tokens, 其長度是變動值，但 <= 512.  儘量塞 sentence 或是 0 padding to 512.  每個 token 是 768-dim (feature) vector. 也就是 (input) token embedding 是一個 variable length (<= 512) 2D matrix X.  最終希望做完 attention 運算還是得到同樣的 shape.
...
* 如果 input-output 只作用或處理在 embedding dimension (i.e shared weight for all input tokens!), 例如 1-dimension convolution, kernel 就是 1x768, channel length = 768.  假設 input 是 3-channel (e.g. RGB or KQV), parameter = 3 x 768^2 = 2M parameters.  顯然也不夠力。同時 each token 都是獨立處理，缺乏 context information!   不過這剛好是 transformer 後半部 MLP 部分。
