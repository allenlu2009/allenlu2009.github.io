...
- **什麽時候 KV cache dominant?  Long context,  How long?   nctx >>  2 embedding size!!!**

![[Pasted image 20241004000033.png]]
...
4. 爲什麽是 KV Cache, 不是 QKV Cache?
5. Q 和 K 對稱嗎？
6. 什麽條件，多出的内存占用和内存頻寬超過模型本身的參數？
7. 節省的計算量，和多出的内存占用和内存頻寬是否有 trade-off?
...
b: batch
s: sequence length, or $n_{ctx}$: context length
h: model input dimension, or embedding
...
但是此時仍然需要計算 softmax (Q K) at t+1, right?
...
是否可能 “時間換空間"?  On-die 7GB 或是 3.5GB SRAM，也就是 re-compute?  不划算！
