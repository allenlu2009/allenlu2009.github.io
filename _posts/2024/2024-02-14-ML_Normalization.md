...
語言模型 (LLM) 的正規化如下圖：注意 layer normalization 的定義不包含 sequence (i.e. time) dimension!  但是 batch normalization 卻是包含 sequence dimension.
...
Batch Normalization 會遇到 training and inferencing batch mismatch 的問題。因爲 training 時用的 batch size 一般比較大 (16/32, ../128)  以平均偏差。但是在 inferencing batch size 可能很小 (batch =1 for device),  或是很大 (data center, batch = 512 for BW efficiency). 
