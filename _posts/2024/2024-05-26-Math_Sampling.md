 ...
 **Markov Chain Monte Carlo (MCMC)** methods, each new sample depends on the previous state, which introduces **dependence between samples**. This violates the assumption of independence required in many statistical applications. However, MCMC addresses this issue through a combination of techniques and properties that ensure the samples can still approximate the target distribution effectively.

**Metropolis-Hastings 演算法** 是統計學和機器學習中廣泛使用的方法，用於在直接抽樣困難時，從概率分佈中進行抽樣。它是一種 **馬可夫鏈蒙特卡羅（Markov Chain Monte Carlo, MCMC）** 方法，通過從目標概率分佈 $p(x)$ 中生成一系列樣本來達成目的，或是目標分佈是以比例常數的形式已知。

以下是這個演算法的關鍵概念和步驟的詳細說明：

---

### **目的**
Metropolis-Hastings 的目標是從概率分佈 $p(x)$ 中生成樣本，特別是在以下情況下：
1. 直接抽樣不可行。
2. 機率分佈 $p(x)$ 僅以比例常數的形式已知，即 $p(x) \propto f(x)$，其中 $f(x)$ 是計算上可用的函數。
...
2. **提議分佈** ($q(x' | x)$)：  
   用於基於當前狀態 $x$ 提議新候選樣本 $x'$ 的分佈。這可以是任何方便的分佈（例如，高斯分佈）。
...
3. **接受比率**：  
   ...
   $$
   如果目標分佈 $p(x)$ 僅以比例常數形式已知，常數項會相互抵消。
...
2. **提議新狀態**：  
   從提議分佈 $q(x' | x_t)$ 中生成候選樣本 $x'$，其中 $x_t$ 是當前狀態。

3. **計算接受比率**：  
   計算接受比率 $A(x' | x_t)$。

4. **接受或拒絕**：  
   - 以概率 $A(x' | x_t)$ 接受 $x'$ 作為下一狀態 $x_{t+1}$。
   - 否則保留當前狀態，即 $x_{t+1} = x_t$。
...
### **優點**
- 即使目標分佈 $p(x)$ 僅以比例常數形式已知，該方法仍然有效。
- 它能處理複雜的高維分佈。

---

### **挑戰**
- **混合時間**：如果提議分佈 $q(x' | x)$ 選得不好，算法可能需要許多迭代才能收斂到目標分佈。
- **提議分佈的選擇**：算法的效率取決於 $q(x' | x)$ 與 $p(x)$ 的形狀匹配程度。
- **拒絕率**：如果提議分佈與目標分佈匹配度不高，許多提議樣本可能會被拒絕。
...
### **範例**
假設我們想從 $p(x) \propto e^{-x^2/2}$（高斯分佈）中抽樣。

1. 設置提議分佈 $q(x' | x)$ 為 $\mathcal{N}(x, \sigma^2)$，即以當前狀態為中心、方差為 $\sigma^2$ 的高斯分佈。
2. 每一步：
   - 提議 $x' \sim \mathcal{N}(x_t, \sigma^2)$。
   - 計算 $A(x' | x_t) = \min\left(1, \frac{p(x')}{p(x_t)}\right)$，因為 $q(x' | x_t) = q(x_t | x')$。
   - 接受或拒絕 $x'$。

---

這個算法保證，隨著時間的推移，樣本 $x_t$ 將近似目標分佈 $p(x)$。
### 總結
