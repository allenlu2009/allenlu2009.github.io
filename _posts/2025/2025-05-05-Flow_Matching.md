...
Ans:  代表我們只看從 $x_t$ 到 $x_1$ 的 trajectories 的機率, 而不是所有到 $x_1$ 到 trajectories.  是因為 causality?  如果沒有經過 $x_t$ 產生的 $x_1$ 對 vector field 毫無貢獻？

可以把 $u_t(x \mid x_1)$ 想成是「若最終目的地是 $x_1$，那麼在 $x$ 時應該往哪個方向走」。

![[Pasted image 20250606234729.png]]
...
所以在 OT case, 每個 $x_0$ 都會先指向 $x_1$ 的平均值。因爲 $x_0$ 和 $x_1$ 完全 independent
$$\begin{aligned}
...
### **Closed-Form Solution for Reversed Means**

Given:
- **Prior distribution $p_0$:** $\mathcal{N}(\mathbf{x}; -\boldsymbol{\mu}, \mathbf{I})$ (mean $-\boldsymbol{\mu}$),
- **Target distribution $p_1$:** $\mathcal{N}(\mathbf{x}; +\boldsymbol{\mu}, \mathbf{I})$ (mean $+\boldsymbol{\mu}$),
- **Linear interpolation path:** $\mathbf{x}_t = t \mathbf{x}_1 + (1-t) \mathbf{x}_0$.

To compute the vector field $\mathbf{u}_t(\mathbf{x})$ that transports samples from the prior distribution $p_0 = \mathcal{N}(\mathbf{x}; -\boldsymbol{\mu}, \mathbf{I})$ to the target distribution $p_1 = \mathcal{N}(\mathbf{x}; +\boldsymbol{\mu}, \mathbf{I})$ along the linear interpolation path $\mathbf{x}_t = t \mathbf{x}_1 + (1-t) \mathbf{x}_0$, we follow the framework from Lipman's paper on conditional flow matching. Here, $\boldsymbol{\mu} = [10, 0]$, and the identity covariance matrix $\mathbf{I}$ implies independence across dimensions.
...
### Derivation:
1. **Marginal Distribution at Time $t$**:
   ...
   $$
   \mathbf{x}_t \sim \mathcal{N}\left( (2t-1)\boldsymbol{\mu}, \sigma_t^2 \mathbf{I} \right) = \mathcal{N}\left( \boldsymbol{\mu}_t, \sigma_t^2 \mathbf{I} \right),
   $$
   where $\sigma_t^2 = (1-t)^2 + t^2 = 2t^2 - 2t + 1$.

2. **Conditional Expectations** (最關鍵一步，Appendix D):
   Using Gaussian conditioning, the posterior expectations given $\mathbf{x}_t = \mathbf{x}$ are:
   $$
   \mathbb{E}[\mathbf{x}_0 \mid \mathbf{x}_t = \mathbf{x}] = -\boldsymbol{\mu} + \frac{1-t}{\sigma_t^2} (\mathbf{x} - (2t-1)\boldsymbol{\mu}),
   $$
   $$
...
To compute $\log p_t(\mathbf{x})$ for the marginal distribution at time $t$, we start from the linear interpolation path $\mathbf{x}_t = t \mathbf{x}_1 + (1-t) \mathbf{x}_0$, with independent samples:
- $\mathbf{x}_0 \sim \mathcal{N}(-\boldsymbol{\mu}, \mathbf{I})$
- $\mathbf{x}_1 \sim \mathcal{N}(+\boldsymbol{\mu}, \mathbf{I})$

Since $\mathbf{x}_t$ is a linear combination of independent Gaussians, it is also Gaussian. We compute its mean and covariance.
...
$$
p_t(\mathbf{x}) = \mathcal{N}\left( \mathbf{x}; (2t-1)\boldsymbol{\mu}, \sigma_t^2 \mathbf{I} \right)
$$

### Log-Density Formula:
For a $d$-dimensional Gaussian $\mathcal{N}(\mathbf{x}; \mathbf{m}, \mathbf{\Sigma})$, the log-density is:
$$
\log p_t(\mathbf{x}) = -\frac{d}{2} \log(2\pi) - \frac{1}{2} \log|\det(\mathbf{\Sigma})| - \frac{1}{2} (\mathbf{x} - \mathbf{m})^\top \mathbf{\Sigma}^{-1} (\mathbf{x} - \mathbf{m})
$$
Substitute $\mathbf{m} = (2t-1)\boldsymbol{\mu}$ and $\mathbf{\Sigma} = \sigma_t^2 \mathbf{I}$:
- $\det(\mathbf{\Sigma}) = (\sigma_t^2)^d$
- $\mathbf{\Sigma}^{-1} = \frac{1}{\sigma_t^2} \mathbf{I}$
- $(\mathbf{x} - \mathbf{m})^\top \mathbf{\Sigma}^{-1} (\mathbf{x} - \mathbf{m}) = \frac{1}{\sigma_t^2} \|\mathbf{x} - (2t-1)\boldsymbol{\mu}\|^2$

Thus:
$$
\log p_t(\mathbf{x}) = -\frac{d}{2} \log(2\pi) - \frac{d}{2} \log(\sigma_t^2) - \frac{1}{2\sigma_t^2} \|\mathbf{x} - (2t-1)\boldsymbol{\mu}\|^2
$$
where $\sigma_t^2 = 2t^2 - 2t + 1$.

### Final Expression:
$$
\boxed{\log p_{t}(\mathbf{x}) = -\dfrac{d}{2} \log(2\pi) - \dfrac{d}{2} \log\left(2t^{2} - 2t + 1\right) - \dfrac{1}{2\left(2t^{2} - 2t + 1\right)} \left\| \mathbf{x} - (2t-1)\boldsymbol{\mu} \right\|^{2}}
$$

### Special Case for $\boldsymbol{\mu} = [10, 0]$ and $d = 2$:
If $\mathbf{x} = [x_1, x_2]^\top$, then:
- $(2t-1)\boldsymbol{\mu} = [10(2t-1), 0]^\top = [20t - 10, 0]^\top$
- $\|\mathbf{x} - (2t-1)\boldsymbol{\mu}\|^2 = (x_1 - 20t + 10)^2 + x_2^2$

Thus:
$$
\log p_t(\mathbf{x}) = -\log(2\pi) - \log(2t^2 - 2t + 1) - \dfrac{(x_1 - 20t + 10)^2 + x_2^2}{2(2t^2 - 2t + 1)}
$$
