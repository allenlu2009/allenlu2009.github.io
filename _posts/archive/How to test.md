...
$$
\begin{aligned}
& \mathbb{E}_{q\left(\mathbf{x}_0\right)}\left[-\log p_\theta\left(\mathbf{x}_0\right)\right] \leq \mathbb{E}_{q\left(\mathbf{x}_0\right) q\left(\mathbf{x}_{1: T} \mid \mathbf{x}_0\right)}\left[-\log \frac{p_\theta\left(\mathbf{x}_{0: T}\right)}{q\left(\mathbf{x}_{1: T} \mid \mathbf{x}_0\right)}\right]=: L \\
& \text { Decoder parameterization: } \quad p_\theta\left(\mathbf{x}_{t-1} \mid \mathbf{x}_t\right)=\mathcal{N}\left(\mathbf{x}_{t-1} ; \mu_\theta\left(\mathbf{x}_t, t\right), \sigma_t^2 \mathbf{I}\right) \\
& \left.\mu_\theta\left(\mathbf{x}_t, t\right)=\frac{1}{\sqrt{1-\beta_t}}\left(\mathbf{x}_t-\frac{\beta_t}{\sqrt{1-\bar{\alpha}_t}} \epsilon_\theta\left(\mathbf{x}_t, t\right)\right) \begin{array}{l}
\begin{array}{l}
\text { Up to scaling, pr } \\
\text { noise that was a } \\
\text { and subtract it }
\end{array} \\
L=\mathbb{E}_{\mathbf{x}_0 \sim q\left(\mathbf{x}_0\right), t \sim \mathcal{U}\{1, T\}, \epsilon \sim \mathcal{N}(0, \mathbf{I})}\left[\lambda_t \| \epsilon-\epsilon_\theta\left(\sqrt{\bar{\alpha}_t} \mathbf{x}_0+\sqrt{1-\bar{\alpha}^{\prime}} \epsilon, t\right)+t^2\right]
\end{array}\right]
\end{aligned}
$$